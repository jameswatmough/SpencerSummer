...
Fixing $\bar{x}$ positive and setting $γ = 1 + 1/\bar{x}$, so that $γ>1$, we can follow the two interesting branches of equilibria as $κ$ increases.  
The prey-only branch, $(κ,0)$, switches from stable to unstable as $κ$ increases through $\bar{x}$,
and second branch, $(\bar{x}, \bar{y}(κ))$, which bifurcates from the first branch at $κ=\bar{x}$ and switches from stable to unstable via a Hopf bifurcation as $κ$ increases through $1+2\bar{x} = (1+γ)/(1-γ)$.
...
xbar = 1
c = 1 + 1/xbar
yinteq = function(K,x=1) (1+x)*(1-x/K)
plot(NULL,xlim=c(0,5),ylim=c(0,2.2),
	xlab=expression(paste("carrying capacity,  ",kappa)),
	ylab = expression(paste(plain("Prey density,  "),y))
#	xlab="carrying capacity, K",
#	ylab="Prey density, P"
)
lines(c(0,1),c(0,0),col="blue")
lines(c(1,5),c(0,0),col="red")
curve(yinteq,1,3,col='blue',type='l',add=TRUE)
curve(yinteq,3,5,col='red',type='l',add=TRUE)
```
